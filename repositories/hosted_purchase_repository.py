...
class HostedPurchaseRepository:
    ...
    def list_by_workspace_user_and_site(
        ...
        """Return purchases for a user+site pair ordered chronologically (ASC)."""
        user_alias = aliased(HostedUserRecord)
        site_alias = aliased(HostedSiteRecord)
        card_alias = aliased(HostedCardRecord)
        query = (
            ...
            .order_by(
                HostedPurchaseRecord.purchase_date.asc(),
                HostedPurchaseRecord.purchase_time.asc(),
                HostedPurchaseRecord.id.asc(),
            )
        )
        rows = session.execute(query).all()
        return [self._row_to_model(row) for row in rows]
