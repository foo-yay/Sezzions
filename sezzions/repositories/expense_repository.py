...
class ExpenseRepository:
    ...
    def get_by_id(self, expense_id: int) -> Optional[Expense]:
        row = self.db.fetch_one("SELECT * FROM expenses WHERE id = ?", (expense_id,))
    ...
    def update(self, expense: Expense) -> Expense:
        ...
        self.db.execute(
            """
            UPDATE expenses
            SET expense_date = ?, amount = ?, vendor = ?, description = ?, category = ?,
    ...
    def _row_to_model(self, row) -> Expense:
        ...
        expense = Expense(
            ...
            category=row["category"] if "category" in row.keys() else "Other Expenses",
