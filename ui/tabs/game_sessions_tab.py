...
class GameSessionsTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.view_button = QPushButton("👁️ View")
        self.end_button = QPushButton("⏹️ End")
            ...
            try:
                ...
                for row, session in enumerate(rows):
                    ...
                    if session.end_date and session.end_date != session.session_date:
                        ...
                        start_date = session.session_date if isinstance(session.session_date, date) else datetime.strptime(session.session_date, "%Y-%m-%d").date()
                        end_date = session.end_date if isinstance(session.end_date, date) else datetime.strptime(session.end_date, "%Y-%m-%d").date()
...
class EditClosedSessionDialog(QDialog):
    ...
    def _load_session(self):
        ...
        if game:
            ...
            # Load game type directly if no game but game_type_id is set
            type_obj = self._game_types_by_id.get(self.session.game_type_id)
            game_type_name = type_obj.name if type_obj else None
        if game_type_name:
            self.game_type_combo.blockSignals(True)
...
class EndSessionDialog(QDialog):
    ...
    def _update_session_details(self):
        ...
        if getattr(self, "auto_redeem_check", None) is not None and self.auto_redeem_check.isChecked():
            self._apply_auto_end_redeemable()
    ...
    def _apply_auto_end_redeemable(self) -> None:
        ...
        end_total = Decimal(str(result))
        auto_end_redeem = self._calculate_auto_end_redeemable(end_total)
        auto_text = f"{auto_end_redeem:.2f}"
        if self.end_redeem_edit.text().strip() != auto_text:
            self.end_redeem_edit.blockSignals(True)
            self.end_redeem_edit.setText(auto_text)
