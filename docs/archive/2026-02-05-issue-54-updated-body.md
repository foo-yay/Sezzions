...
## Update (2026-02-05): Coordination notes (based on recent balance-chain work)

These notes were added after recent improvements to balance-chain / expected-balance validation in Purchase + Session dialogs.

Key clarifications / guardrails:

- **Balance checkpoints must apply consistently anywhere Sezzions computes “expected balance”** (not only in Session dialogs).
  - This includes purchase-related balance checks/warnings and any other UI/service path that calls into expected-balance computations.
- **Define deterministic ordering at identical timestamps** (checkpoint vs purchase/session/redemption). This prevents flaky “expected” results when times are minute-granularity or duplicated.
- **Normalize times**: treat NULL `effective_time` as `00:00:00`, and prefer storing `HH:MM:SS` consistently.
- **UI pitfall (Qt mnemonics):** `&` is a mnemonic marker in Qt widget text. Use `&&` to display a literal `&` in button/label captions.
...
- Integration points (important):
  - Balance checkpoint adjustments must affect BOTH:
    ...
    - **purchase expected-balance checks/warnings** (anywhere the Purchase dialogs compute an expected balance or validate a continuity chain)
...
- Integrate adjustments into:
  ...
  - **purchase expected-balance checks/warnings** anywhere expected totals/redeemable are computed at a cutoff
...
- Define an explicit ordering rule when a checkpoint adjustment shares the exact datetime with another event.
- Recommendation for MVP: “Checkpoint at datetime $T$ is considered a valid checkpoint **at** $T$ and wins as the anchor for expected-balance computations at $T$.” (Alternative rules are fine, but must be explicit and tested.)

**Time normalization:**
- Treat NULL `effective_time` as `00:00:00`.
- Prefer storing normalized time strings (`HH:MM:SS`) for adjustments.

MVP simplification:
- Only checkpoint adjustments affect expected computation, not delta SC adjustments.

Also update/extend whichever shared expected-balance helper is used by Purchase dialogs:
- Balance checkpoint adjustments must affect purchase expected-balance checks/warnings too.
...
- **Balance checkpoints affect expected-balance checks consistently across the app** (sessions + purchases).
...
- **Ordering edge case:** checkpoint adjustment at the exact same datetime as a session/purchase cutoff behaves deterministically per the chosen ordering rule.
...
- **Explicitly assert an unrelated (user, site) pair’s derived rows are unchanged** (counts + key computed values).
...
- **Qt mnemonic pitfall:** avoid raw `&` in button labels; use `&&` if needed.
