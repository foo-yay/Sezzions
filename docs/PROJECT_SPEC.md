...
### 5.2 QTableView Migration (Issue #15, Partial)

**Purpose:**
Optional refactor of grid tabs from `QTableWidget` → `QTableView` + `QAbstractTableModel`:
...
- Inline editing is complex and risky; keeping all edits in dialogs (purposeful, validated, recalc-aware)
- Migration is now optional architectural improvement, not prerequisite for editing

**Architecture:**

*Base Model Infrastructure:*
- `ui/base_table_model.py` (125 lines):
  - `ColumnDefinition`: Metadata class defining columns (label, key, formatter, alignment, width_hint)
  - `BaseTableModel(QAbstractTableModel)`: Shared base class for grid tabs
    - Object-oriented: rows are domain objects, not raw strings
    ...
    - `data(index, role)`: Handles DisplayRole, TextAlignmentRole, UserRole (for IDs)
    - `flags(index)`: Returns `ItemIsEnabled | ItemIsSelectable` (no editing support)
  - Subclasses override `data()` for custom formatting/coloring (e.g., inactive status)

*Spreadsheet UX Support:*
- `ui/spreadsheet_ux.py` extended to support QTableView:
  - `_extract_tableview_selection()`: Extract selection from QTableView via `selectedIndexes()`
  - Works with proxy models (QSortFilterProxyModel)
  - Preserves all Phase 1 spreadsheet UX features (copy/stats/context menu)

*Migrated Tabs (Pilot):*
- **Sites tab** (`ui/tabs/sites_tab.py`):
  - Migrated from QTableWidget → QTableView
  - `SitesTableModel(BaseTableModel)`: 5 columns (Name, URL, SC Rate, Status, Notes)
  - `QSortFilterProxyModel` for sorting (column filtering not yet implemented)
  - Removed `TableHeaderFilter` (was QTableWidget-specific)
  - Spreadsheet UX fully preserved (copy/stats/selection behavior identical)
  - All toolbar actions work (add/edit/delete/export)
  - Tests: 5 headless UI tests covering instantiation, model access, filtering, spreadsheet UX

*Remaining Tabs (Not Yet Migrated):*
- Setup tabs (6): Users, Cards, Redemption Method Types, Redemption Methods, Game Types, Games
- Non-Setup tabs (5): Purchases, Redemptions, Game Sessions, Expenses, Unrealized

**Migration Pattern (per tab):**
```python
# 1. Define model subclass
class MyTabTableModel(BaseTableModel):
    def __init__(self, parent=None):
        columns = [
            ColumnDefinition("Name", "name"),
            ColumnDefinition("Rate", "rate", lambda v: f"{v:.2f}", QtCore.Qt.AlignRight),
            ColumnDefinition("Active", "is_active", lambda v: "Yes" if v else "No"),
        ]
        super().__init__(columns, parent)
    
    # Override data() if custom formatting/colors needed
...
# 2. Replace QTableWidget with QTableView + model + proxy
self.model = MyTabTableModel()
...
def refresh_data(self):
    objects = self.facade.get_all_objects()
    self.model.set_data(objects)  # No manual row-by-row population

# 4. Update selection/ID extraction (map through proxy)
def _get_selected_ids(self):
    ids = []
    for proxy_row in self._get_selected_row_numbers():
        proxy_index = self.proxy.index(proxy_row, 0)
        source_index = self.proxy.mapToSource(proxy_index)
        obj = self.model.get_row_object(source_index.row())
        ids.append(obj.id)
    return ids
```

**Benefits:**
- Cleaner code: no manual row/column population loops
- Sorting/filtering handled by proxy model (no re-populating)
- Type safety: work with domain objects, not raw strings
...
**Decision:**
- Sites tab migration successful (pilot)
- Remaining tabs will be migrated incrementally in future work
- All new grid tabs should prefer QTableView architecture
