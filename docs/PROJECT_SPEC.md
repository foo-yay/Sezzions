...
- The repository now includes a real web frontend scaffold under `web/` using Vite + React.
- Canonical frontend build output is `web/dist`.
- Canonical frontend build command is `cd web && npm ci --cache .npm-cache && npm run build`.
...
- The first hosted business-domain data slice is a workspace-owned users directory exposed through `GET /v1/workspace/users`, `POST /v1/workspace/users`, `PATCH /v1/workspace/users/{user_id}`, and `DELETE /v1/workspace/users/{user_id}`.
...
- `GET /v1/workspace/users`, `POST /v1/workspace/users`, `PATCH /v1/workspace/users/{user_id}`, and `DELETE /v1/workspace/users/{user_id}` must require a bearer token, resolve the authenticated user's hosted workspace, and only list/create/update/delete business-domain users within that workspace.
...
- Hosted operational status details should remain available but should not dominate the primary workspace surface; the signed-in shell should expose them through a compact green/orange/red status affordance that opens a modal with the detailed checks.
- The hosted signed-in header should remain visually compact and desktop-like: a small product title plus utility-style notifications, status, account, and settings entry points rather than a large banner-style top shell.
- The hosted Users view dialog should maintain parity with the desktop view dialog by allowing direct transition into Edit from View and preserving explicit delete intent from the same view context.
